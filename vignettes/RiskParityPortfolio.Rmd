...
max_sharpe_ratio <- function(dataset) {
    ...
    return(as.vector(result$getValue(weights)/sum(result$getValue(weights))))
}

# call portfolioBacktest and benchmark against the uniform (1/N) portfolio
bt <- portfolioBacktest(list("risk parity" = riskparity, "tangency" = max_sharpe_ratio),
                        list(faang_data), optimize_every = 60, rebalance_every = 60,
                        benchmark = 'uniform')
