...
The general solver seems to be sensitive to the initial point:
```{r, message=FALSE}
...
res_gen_solver2$w
last(res_gen_solver2$obj_fun)
res_gen_solver2 <- riskParityPortfolioGenSolver(Sigma, b, w0 = c(1, 1, 1)/3, formulation = "rc-over-var-vs-b")
res_gen_solver2$w
last(res_gen_solver2$obj_fun)
```
The SCA method seems to be more robust to the initial point (but still somehow sensitive since it is a nonconvex problem):
```{r}
...
res_sca2$w
last(res_sca2$obj_fun)
res_sca2 <- riskParityPortfolioSCA(Sigma, b, w0 = c(1, 1, 1)/3, formulation = "rc-over-var-vs-b")
res_sca2$w
last(res_sca2$obj_fun)
```
...
res_gen_solver3 <- riskParityPortfolioGenSolver(Sigma, b, w0 = b, formulation = "rc-over-sd-vs-b-times-sd")
res_gen_solver3$w
last(res_gen_solver3$obj_fun)
...
res_sca3 <- riskParityPortfolioSCA(Sigma, b, w0 = b, formulation = "rc-over-sd-vs-b-times-sd")
res_sca3$w
last(res_sca3$obj_fun)
