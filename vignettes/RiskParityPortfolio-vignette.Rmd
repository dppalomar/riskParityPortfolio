...
As it can be observed, our implementation is quite faster than the interior-point method
...
additional constraints and terms desired in practice, such as the mean return, box constraints, etc,
...
Sigma <- V %*% t(V)
mu <- runif(N)
w0 <- riskParityPortfolioNewton(Sigma)$w
...
plot(risk_parity_sca, mean_return_sca, type = "b", pch=19, cex=.6, col=colors[1],
     ...
     main = "Expected Return x Risk for SCA and General Solver algorithms")
