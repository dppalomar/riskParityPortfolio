...
Now, let's see a comparison, in terms of computational time, of our Newton implementation
...
risk parity weights, just like `rp()`. However, users are advised to use the functions
`riskParityPortfolioNewton()` or `riskParityPortfolio()`, which contains more information
such as objective function value and the individual risk contributions associated with each asset.
...
Let's explore, for instance, the effects of having the expected return in the optimization problem.
We recall that the optimization problem that include the expected return can be expressed as
$$\begin{array}{ll}
...
\end{array}$$
in which $R$ is the risk concentration function, $\mathbf{w}^{T}\boldsymbol{\mu}$ is the expected return,
and $\lambda$ is a trade-off parameter.

The code below designes portfolios using both `riskParityPortfolioSCA()` and
`riskParityPortfolioGenSolver()` and compares their results in terms of
risk versus versus expected return.
...
Let's also compare the computational time between `riskParityPortfolioSCA()`
and `riskParityPortfolioGenSolver()`:
...
op <- microbenchmark(sca = riskParityPortfolioSCA(Sigma, w0 = w0, mu = mu, lambda = .5),
                     gensolver = riskParityPortfolioGenSolver(Sigma, w0 = w0, mu = mu, lambda = .5),
                     times = 10L)
print(op)
par(mar = c(7, 4, 4, 2))
boxplot(op, ylab = "Time in Milliseconds", xlab = NULL, unit = "ms", outline = FALSE, las = 2)
...
The SCA method turns out to be faster, which might be explained by the fact that
it solves quadratric programming problems, which are computationally cheap, at every iteration.
...
We also provide a general function called `riskParityPortfolio()` which returns either the result
of `riskParityPortfolioSCA()` initilizaed by the solution of `riskParityPortfolioNewton()`, in case
the expected return or additional constraints are desired, or the solution of
`riskParityPortfolioNewton()` itself for the vanilla formulation.

# Appendix: Risk concentration formulations
