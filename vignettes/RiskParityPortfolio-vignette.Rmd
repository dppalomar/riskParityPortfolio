...
against the `rp()` function from the `cccp` package. (For a fair comparison, instead of calling our function
`riskParityPortfolio()`, we call direclty the core internal function `risk_parity_portfolio_nn()`, which only computes the
risk parity weights, just like `rp()`.)
```{r}
...
and `riskParityPortfolioGenSolver()`c
...
it solves a sequence of quadratic convex problems, which are computationally cheap.
