...
\end{array}$$
where the risk term is of the form (double summation)
$$R(\mathbf{w}) = \sum_{i,j=1}^{N}(g_{ij}(\mathbf{w}))^{2}$$
or simply (single summation)
...
which can be coded as `risk_grad <- 4*(t(Sigma*w) + diag(Sigma %*% w)) %*% (N*r-sum(r))`. An better way to code it is:
...
- $\textsf{J}_{\mathbf{w}}\mathbf{r} = \textsf{Diag}(\mathbf{w})\boldsymbol{\Sigma} + \textsf{Diag}(\boldsymbol{\Sigma}\mathbf{w})$
...
r_b <- r/sum_r - b
v <- 2*r_b - sum(r_b^2)
risk_grad <- Sigma %*% (w*v) + Sigma_w*v
```

For the SCA method, the risk deviation of the $i$-th asset is
$$
\begin{equation}
g_i(\mathbf{w}) = \dfrac{r_i}{\sqrt{\mathbf{1}^{T}\mathbf{r}}} - b_i\sqrt{\mathbf{1}^{T}\mathbf{r}}
\end{equation}
$$
...
\dfrac{r_i}{2 (\mathbf{1}^{T}\mathbf{r})^{3/2}} - \dfrac{b_i}{2\sqrt{\mathbf{1}^{T}\mathbf{r}}}.
\end{equation}
$$
...
- \dfrac{1}{2}\left(\dfrac{\mathbf{r}}{\mathbf{1}^{T}\mathbf{r}} + \mathbf{b} \right)\right).
\end{equation}
$$
...
- \dfrac{1}{2}\left(\dfrac{\mathbf{1}\otimes\mathbf{r}^{T}}{\mathbf{1}^{T}\mathbf{r}} + \mathbf{1}\otimes\mathbf{b}^{T} \right)\right)
\end{equation}
$$
...
\textsf{Diag}(\boldsymbol{\Sigma}\mathbf{w})\right) \cdot \dfrac{1}{{\sqrt{\mathbf{1}^{T}\mathbf{r}}}} \left(\mathbf{I} - \dfrac{1}{2}\left(\dfrac{\mathbf{1}\otimes\mathbf{r}^{T}}{\mathbf{1}^{T}\mathbf{r}} + \mathbf{1}\otimes\mathbf{b}^{T} \right)\right)
\end{equation}
$$
