...
# Appendix IV: Numerical algorithms for the risk-parity portfolio
...
This method simply minimizes in a cyclical manner with respect to each element of the variable $\mathbf{x}$ (denote $\mathbf{x}_{-i}=[x_1,\cdots,x_{i-1},0,x_{i+1},\cdots,x_N]^T$).
...
* For the function $f(\mathbf{x}) = \frac{1}{2}\mathbf{x}^{T}\boldsymbol{\Sigma}\mathbf{x} - \mathbf{b}^T\log(\mathbf{x})$, the minimization w.r.t. $x_i$ is
    $$\underset{x_i>0}{\textsf{minimize}} \quad \frac{1}{2}x_i^2\sigma_i^2 + x_i(\mathbf{x}_{-i}^T\boldsymbol{\Sigma}_{:,i}) - b_i\log{x_i}$$
with gradient $\nabla_i f = x_i\sigma_i^2 + (\mathbf{x}_{-i}^T\boldsymbol{\Sigma}_{:,i}) - b_i/x_i$. Setting the gradient to zero gives us the second order equation
$$x_i^2\sigma_i^2 + x_i(\mathbf{x}_{-i}^T\boldsymbol{\Sigma}_{:,i}) - b_i = 0$$
with positive solution given by
$$x_i^\star = \frac{-(\mathbf{x}_{-i}^T\boldsymbol{\Sigma}_{:,i})+\sqrt{(\mathbf{x}_{-i}^T\boldsymbol{\Sigma}_{:,i})^2+4\sigma_i^2b_i}}{2\sigma_i^2}.$$

* For the function $f(\mathbf{x}) = \sqrt{\mathbf{x}^{T}\boldsymbol{\Sigma}\mathbf{x}} - \mathbf{b}^T\log(\mathbf{x})$, XXX
...
approximation (SCA). The SCA method works by linearizing the risk concentration term at some
pre-defined point, casting the nonconvex problem into a much simpler strictly convex
optimization problem. This procedure is then iterated until convergence is achieved. It's important
to highlight that the SCA method always converge to a stationary point.

At the k-th iteration, the SCA method aims to solve
\begin{align}\begin{array}{ll}
    \underset{\mathbf{w}}{\textsf{minimize}} & \sum_{i=1}^{n}\left(g_i(\mathbf{w}^k) +
    (\nabla g_i(\mathbf{w}^{k}))^{\intercal}(\mathbf{w} - \mathbf{w}^{k})\right)^2 +
    \tau ||\mathbf{w} - \mathbf{w}^{k}||^{2}_{2} + \lambda F(\mathbf{w})\\
\textsf{subject to} & \mathbf{w}^{T}\mathbf{1} = 1, \mathbf{w} \in \mathcal{W},
\end{array}\end{align}
where the term under the summation is the first order Taylor expasion of the risk concentration
$R$.
...
## Vinicius's Roncalli's Formulation
The risk parity formulation proposed in [@Billion2013] is 
$$\begin{array}{ll}
...
Since $w_i > 0$ and $\boldsymbol{\Sigma}_{ii} > 0$, then the LHS of the expression above
is a convex second-order polynomial with respect to $w_i$. Also note that its discriminant,
$\Delta$, is given by
$$\begin{equation}
...
## Viniciu's Spinu's Formulation
In [@Spinu2013], Spinu proposed the following risk parity formulation:
$$\begin{array}{ll}
...
In a similar fashion, the cyclical algorithm for this formulation
has the exact same structure as in the previous case, but with
$\sigma(\mathbf{w}) = 1$ and $\boldsymbol{\Sigma}\mathbf{w}$ updated as
