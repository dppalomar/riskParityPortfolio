...
Others packages with the goal of designing risk parity portfolios do exist,
such as `FinCovRegularization` and `cccp`. Let's check how do they perform
against the functions we present here.
...
As it can be observed, the function `RiskParity` from the `FinCovRegularization`
package fails to design a portfolio that meet the risk parity requirement, even
in a scenario with as few as ten assets. As for the `cccp` package, although it
works as expected, its API is cumbersome when compared to our straightforward
function call.

Now let's take a look at the computational time taken by our functions and th
`rp` function from the `cccp` package:
...
In the sections that follows we show the computational time required by
...
lines(risk_parity_gen, mean_return_gen, type = "b", pch=19, cex=.6, col = colors[2],
      xlab = "Risk Parity", ylab = "Mean Return")
legend("bottomright", legend=c("SCA", "GenSolver"), col=colors, lty=c(1, 1, 1), cex=0.8)
```
