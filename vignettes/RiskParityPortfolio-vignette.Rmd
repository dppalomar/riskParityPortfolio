...
op <- microbenchmark(rp_cccp = rp(b, Sigma, b, optctrl = ctrl(trace = FALSE)),
                     cyclical = riskParityPortfolio:::risk_parity_portfolio_ccd_spinu(Sigma, b,
                                                                                      1e-6, 50),
                     rp_optimalPortfolio = optimalPortfolio(Sigma = Sigma,
...
riskport_w <- optimalPortfolio(Sigma = Sigma, control = list(type = 'erc', constraint = 'lo'))
...
barplot(rbind(rpp$w, fincov_w, cccp_w, riskport_w),
        ...
        legend = c("riskParityPortfolio", "FinCovRegularization", "cccp", "RiskPortfolios"))
...
and `optimalPortfolio()` from the `RiskPortfolios` package, the other functions perform the same.
`FinCovRegularization` uses a general solver with a hard coded initial guess for the portfolio
weights. We conjecture that its poor initialization might be the reason for the convergence to a local solution.
...
and [@Billion2013], respectively. Nonetheless, they output the same solution.
```{r}
library(microbenchmark)
library(riskParityPortfolio)
...
for (i in size_seq) {
  ...
  Sigma <- V %*% t(V)
  bench <- microbenchmark(newton = riskParityPortfolio(Sigma, method_init="newton"),
