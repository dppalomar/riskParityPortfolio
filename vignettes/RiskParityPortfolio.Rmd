...
max_sharpe_ratio <- function(dataset) {
    ...
    if (all(mu <= 1e-8)) w <- rep(0, nrow(Sigma))
    else {
      weights <- CVXR::Variable(nrow(Sigma))
      constraints <- list(t(mu) %*% weights == 1, weights >= 0)
      problem <- CVXR::Problem(CVXR::Minimize(CVXR::quad_form(weights, Sigma)),
                               constraints = constraints)
      result <- solve(problem)
      w <- as.vector(result$getValue(weights)/sum(result$getValue(weights)))
    }
    return(w)
}

library(quadprog)
quadprog_tangency <- function(dataset) {
    prices <- dataset$adjusted
    log_returns <- diff(log(prices))[-1]
    Sigma <- cov(log_returns)
    mu <- colMeans(log_returns)
...
bt <- portfolioBacktest::portfolioBacktest(list("risk parity" = riskparity,
                                                "tangency"    = max_sharpe_ratio,
                                                "quadprog-tangency" = quadprog_tangency),
                                           list(faang_data),
...
backtestChartStackedBar(bt, portfolio = names(bt[1]), legend = TRUE)
backtestChartStackedBar(bt, portfolio = names(bt[2]), legend = TRUE)
backtestChartStackedBar(bt, portfolio = names(bt[3]), legend = TRUE)
```

Indeed, the charts are quite similar to those reported in [@souza2019].
