function [w_k] = RP_Regularized_Quadratic(M, w0, tau, TOParam, CovMatrix, ApproxFun, MaxIter)
...
% set container
rec_val = [];
rec_val = [rec_val PO_Objective(w_k, theta_k, CovMatrix, TOParam, ApproxFun)];
...
%     blx = [];
    ...
    while 1
        ...
        obj_next = PO_Objective(w_next, theta_next, CovMatrix, TOParam, ApproxFun);
        if (obj_next <= obj_cur)
            break;
        else
            tmpgam = tmpgam * 0.9;
