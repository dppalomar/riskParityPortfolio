...
We provide two functions to design risk parity portofolios: *riskParityPortfolioSCA* and *riskParityPortfolioGenSolver*.
*riskParityPortfolioSCA* makes use of the successive convex approximation framework in order to optimize the portfolio weights,
whereas *riskParityPortfolioGenSolver* uses general solvers such as slsqp and alabama.
...
# compute uniform initial guess for the portfolio weights
w0 <- rep(1 / N, N)
...
barplot(rbind(sca$w, slsqp$w, alabama$w),
        main = "Portfolio Weights", xlab = "stocks", ylab = "dollars", beside = TRUE,
        col = rainbow8equal[1:3], legend = c("sca", "slsqp", "alabama"))
...
barplot(rbind(sca$risk_contribution, slsqp$risk_contribution, alabama$risk_contribution),
        main = "Risk Contribution of the Portfolios", xlab = "stocks", ylab = "dollars", beside = TRUE,
        col = rainbow8equal[1:3], legend = c("sca", "slsqp", "alabama"))
```

Please, check the documentation for the full description of the parameters taken by each function.

# Performance comparison with other packages

Others packages with the goal of designing risk parity portfolios do exist, such as *FinCovRegularization* and
*cccp*. Let's check how do they perform against the functions we present here.
...
cccp_rp <- c(getx(rp(w0, Sigma, w0, optctrl = ctrl(trace = FALSE))))
cccp_risk_contribution <- c(cccp_rp * (Sigma %*% cccp_rp))

barplot(rbind(sca$w, slsqp$w, alabama$w, fincov, cccp_rp),
        main = "Portfolios Weights", xlab = "stocks", ylab = "dollars", beside = TRUE,
        col = rainbow8equal[1:5], legend = c("sca", "slsqp", "alabama", "FinCovRegularization", "cccp"))
...
As it can be observed, the function *RiskParity* from the *FinCovRegularization* package fails to
design a portfolio that meet the risk parity requirement, even in a scenario with as few as ten assets.
As for the *cccp* package, although it works as expected, its API is cumbersome when compared to our
straightforward function call.

Now let's take a look at the computational time taken by our functions and the *rp* function from the *cccp* package:
...
Although the  *rp* function from the *cccp* package is around 6x faster than *riskParityPortfolioSCA*,
*cccp* uses a formulation that is only applicable for long-only portfolio, whereas the formulations available
in our package can be applied for the design of general long/short portfolios.
...
```{r}
res_slsqp <- riskParityPortfolioGenSolver(Sigma, formulation = "rc vs b-times-var",
                                          method = "slsqp")
res_slsqp_nograd <- riskParityPortfolioGenSolver(Sigma,
                                                 formulation = "rc vs b-times-var",
                                                 method = "slsqp",
...
```{r}
res_slsqp <- riskParityPortfolioGenSolver(Sigma, formulation = "rc-over-sd vs b-times-sd",
                                          method = "slsqp")
res_slsqp_nograd <- riskParityPortfolioGenSolver(Sigma,
                                                 formulation = "rc-over-sd vs b-times-sd",
                                                 method = "slsqp",
