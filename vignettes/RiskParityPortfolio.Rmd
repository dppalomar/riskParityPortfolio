...
#rpp_mu <- riskParityPortfolio(Sigma, formulation = "rc-double-index",
rpp_mu <- riskParityPortfolio(Sigma, b = rep(1/nrow(Sigma), nrow(Sigma)), formulation = "rc-over-b-double-index",
...
## Markowitz
Markowitz proposed in his seminal 1952 paper [@Markowitz1952] to find a trade-off between the portfolio expected return and its risk measured by the variance:
$$\begin{array}{ll}
...
# plot(risk_parity, mean_return, type = "b", pch = 19, cex = .6, col = "#ff7675",
...
ggplot(data.frame(risk_parity, mean_return), aes(x = risk_parity, y = mean_return)) +
  geom_line() + geom_point() +
  labs(title = "Expected Return vs Risk Parity", 
       x = "Risk Parity", y = "Expected Return")
```
...
for (lmd_var in lmd_sweep) {
  rpp <- riskParityPortfolio(Sigma, lmd_var = lmd_var, formulation = "rc-double-index")
  variance <- c(variance, rpp$variance)
  risk_parity <- c(risk_parity, rpp$risk_parity)
}
volatility <- sqrt(variance)

# plot(risk_parity, volatility, type = "b", pch = 19, cex = .6, col = "#ff7675",
#      xlab = "Risk Parity", ylab = "Volatility",
#      ylim = c(min(volatility), max(volatility)),
#      xlim = c(min(risk_parity), max(risk_parity)),
#      main = "Volatility vs Risk Parity")
ggplot(data.frame(risk_parity, volatility), aes(x = risk_parity, y = volatility)) +
  geom_line() + geom_point() +
  labs(title = "Volatility vs Risk Parity", 
       x = "Risk Parity", y = "Volatility")
```
...
# design portfolio
rpp <- riskParityPortfolio(Sigma, method = "sca", Dmat = Dmat, dvec = dvec)
...
op <- microbenchmark(
          ...
          riskParityPortfolio = riskParityPortfolio:::risk_parity_portfolio_ccd_spinu(Sigma, b, 1e-6, 50),
          RiskPortfolios = optimalPortfolio(Sigma = Sigma, 
...
df <- data.frame(size_seq, newton = times[1, ], cyclical_spinu = times[2, ], cyclical_roncalli = times[3, ])
molten_df <- reshape2::melt(df, id.vars = "size_seq")
ggplot(molten_df, aes(x = size_seq, y = value, col = variable)) + 
  geom_line() + geom_point() + 
