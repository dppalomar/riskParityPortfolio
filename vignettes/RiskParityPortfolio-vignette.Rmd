...
Sigma <- diag(c(1:10) ^ 2)
mu <- runif(10)
res <- riskParityPortfolio(mu, Sigma, nu = 0., type = "1", ftol = 1e-4)
niter <- length(res$obj_fun)
par(mfrow = c(1, 2))
plot(stock_index, res$portfolio_weights, type = "b", pch=19, cex=.6,
     col = scales::alpha("black", .5), xlab = "Stock number",
     ylab = "Portfolio weights")
plot(c(1:niter), res$obj_fun, type = "b", pch=19, cex=.6,
